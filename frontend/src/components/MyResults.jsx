...
// show Badge account to the display results
const Badge = ({ percent }) => {
  if (percent >= 85)
...
const MyResults = ({ apiBase = "http://localhost:8000" }) => {
      ...
      try {
        const q =
          tech && tech.toLowerCase() !== "all"
            ...
            : "";
        const res = await axios.get(`${apiBase}/api/vi/results${q}`, {
  ...
  useEffect(() => {
    ...
    const fetchAllForTechList = async () => {
      try {
        const res = await axios.get(`${apiBase}/api/vi/results`, {
          headers: { "Content-Type": "application/json", ...getAuthHeader() },
          timeout: 10000,
        });
        if (!mounted) return;
    ...
    <div className={resultStyles.pageContainer}>
      <div className={resultStyles.container}>
        ...
        <div className={resultStyles.filterContainer}>
          <div className={resultStyles.filterContent}>
            <div className={resultStyles.filterButtons}>
              ...
              <button onClick={() => handleSelectTech('all')} className={`${resultStyles.filterButton} ${selectedTechnology === 'all' ? resultStyles.filterButtonActive
        ...
        {loading ? (
          ...
          <>
            {Object.entries(grouped).map(([track, item]) => {
            ...
            {Array.isArray(results) && results.length === 0 && !error && (
              <div className={resultStyles.emptyState}>
                No results yet .Take a quiz a see result here
              </div>
            )}
          </>
        )}
